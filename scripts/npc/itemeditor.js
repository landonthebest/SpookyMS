...
let tempsel2;
let tempselIns;
let mobid = 100100; // default mob id, green snail
let itemid;
let chance;
let items;
let chances;
let exception = false; // dont touch this boolean
let debug = false; // set to true if the itemlist of a mob crashes you
let droprate = 1;
...
function search() {
    ...
    for (let i = 0; i < items.size(); i++) {
        if (items.get(i) != 0) {
            ...
            text += "\r\n#L" + i + "##" + (debug ? "t" : "i") + "" + items.get(i) + "# #b#z" + items.get(i) + "##k" + (dropChance > 100 ? "100" : dropChance) + "%#l";
        }
    }
    cm.sendSimple(text);
}
...
function update() {
    if (cm.masterQueryUpdate("UPDATE drop_data SET chance = " + chance * 2 + " WHERE dropperid = " + mobid + " AND itemid = " + itemid + "")) {
        ...
        cm.sendOk("Something went wrong! (alert Dev)");
    }
}
