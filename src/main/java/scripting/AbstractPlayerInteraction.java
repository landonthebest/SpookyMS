...
import server.PlayerRingStatsManager;
...
public class AbstractPlayerInteraction {
    ...
    // AdventureMS / CosmicMS Custom
    public void gainEquip(Equip equip, boolean showMessage) {
        if (equip == null) return;
        int itemId = equip.getItemId();
        short quantity = 1; // Equips are always quantity 1
        // Inventory check
        if (!InventoryManipulator.checkSpace(c, itemId, quantity, "")) {
            c.getPlayer().dropMessage(1, "Your equip inventory is full. Please remove an item.");
            return;
        }
        InventoryManipulator.addFromDrop(c, equip, false);
        if (showMessage) {
            c.sendPacket(PacketCreator.getShowItemGain(itemId, quantity, true));
        }
    }

    public void gainEquip(Equip equip) {
        gainEquip(equip, true);
    }

    public void updateRingStats(int ringId, int jump, int speed) {
        // Update equipped ring(s)
        Inventory equippedInv = getPlayer().getInventory(InventoryType.EQUIPPED);
        for (Item item : equippedInv.list()) {
            if (item.getItemId() == ringId && item instanceof Equip equip) {
                equip.setJump((short) jump);
                equip.setSpeed((short) speed);
                // No saveToDB() needed
                c.sendPacket(PacketCreator.modifyInventory(false, Collections.singletonList(new ModifyInventory(1, equip))));
            }
        }
        // Update inventory ring(s)
        Inventory equipInv = getPlayer().getInventory(InventoryType.EQUIP);
        ...
        PlayerRingStatsManager.saveStats(getPlayer().getId(), ringId, jump, speed);
