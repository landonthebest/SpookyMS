    ...
    public void handlePacket(InPacket p, Client c) {
        ...
        if (obj instanceof NPC npc) {
            ...
            if (npc.getId() == NpcId.DUEY) {
                ...
                if (npc.getId() == 9800052) {
                    NPCScriptManager.getInstance().start(c, npc.getId(), "slayer", null);
                    return;
                }
                if (npc.getId() == 9800051) {
                    NPCScriptManager.getInstance().start(c, npc.getId(), "reddog", null);
                    return;
                }
                if (npc.getId() == 9100200) {
                    NPCScriptManager.getInstance().start(c, npc.getId(), "blackjack", null);
                    return;
                }
                if (npc.getId() == 9800048) {
                    NPCScriptManager.getInstance().start(c, npc.getId(), "ringequip", null);
                    return;
                }
                // Custom handling to reduce the amount of scripts needed.
                if (npc.getId() == 9800050) {
                    NPCScriptManager.getInstance().start(c, npc.getId(), "slotmachine", null);
                    return;
                }
                if (
                        (npc.getId() >= NpcId.GACHAPON_MIN && npc.getId() <= NpcId.GACHAPON_MAX)
                                || (npc.getId() >= 9800036 && npc.getId() <= 9800049) // Support custom Gachapon range
                ) {
